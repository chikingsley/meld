...
import { expressionColors, isExpressionColor } from "@/components/chat-window/expressions-utils/expressionColors";
import { expressionLabels } from "@/components/chat-window/expressions-utils/expressionLabels";
    ...
    <div className="text-xs p-3 w-full border-t border-border flex flex-col md:flex-row gap-3">
      {top3.map(([key, value]) => (
        <div key={key} className={"w-full overflow-hidden"}>
          ...
          <div className="relative h-1" style={{ "--bg": isExpressionColor(key) ? expressionColors[key] : "var(--bg)" } as CSSProperties}>
            ...
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${R.pipe(value, R.clamp({ min: 0, max: 1 }), (value) => `${value * 100}%`)}` }}
            />
          </div>
        </div>
      ))}
